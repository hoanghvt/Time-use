...
func_boot_VZV<-function(Imp_index,location_tag,location_save){
  contact_all_2<-get(load(file=file.path(TUD_matrice_folder,"contact_all_2.Rdata")))
  contact_all_2<-contact_all_2[,-1]
  general<-get(load(file=file.path(TUD_matrice_folder,paste0(location_tag,Imp_index,".RData"))))
  
  contact.result<-contact.fitter(a=a,y=y,rij1=contact_all_2, rij2=general,muy=muy,N=N,D=D,Lmax=85,plots="TRUE",startpar=c(5e-2,0.84))
...
for(Imp_index in 1:10){
  func_boot_VZV(Imp_index,"aij_general_MI","VZV_combined_MI") 
}
