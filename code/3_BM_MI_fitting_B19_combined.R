...
func_boot_B19<-function(Imp,boot){
  ...
  TUD<-time_use[[boot]]
  contact.result<-contact.fitterV2MSIRWb(a=a,y=y,rij1=contact_all,rij2=TUD,muy=muy,N=N,D=D,Lmax=85,plots="TRUE",startpar=c(0.22,0.54,0.083))
...
foreach(Imp=1:10,
  ...
  foreach(boot = 1:1000,   # iteration index
          ...
          .errorhandling = 'remove')  %dopar% # transfer packages and run parallel  
            ...
            {
              result<-func_boot_B19(Imp,boot)  
            }-> result
    filename = paste0("BM_combined_B19_",Imp, ".txt")
