...
##### CONTACT.FITTER #######
contact.fitter<-function(a,y,rij1,rij2,muy,N,D,Lmax,plots="TRUE",startpar){
  L<-Lmax*mean(exp(-cumsum(muy)))
  qproc<-function(a,y,qpar,rij,Lmax,N,D,plots="TRUE"){
    if (Lmax>100){return("Please specify Lmax<100")}
    rij<-rij1*(1-exp(-qpar[2]*rij2/rij1))
    bij<-365*qpar[1]*(rij)[1:Lmax,1:Lmax]
  ...
  return(list(hessian=q.result$hessian,FOI=result.global$foii,pi=result.global$pi,qhat=q.result$estimate,deviance=q.result$minimum,aic=q.result$minimum+sum(q.result$estimate!=0)*2,bic=q.result$minimum+sum(q.result$estimate!=0)*log(length(y)),bij=result.global$bij,R0=max(as.double(result.global$eivalues))))
  ...
  #return(list(hessian=q.result$hessian,FOI=result.global$foii,pi=result.global$pi,qhat=q.result$par,deviance=q.result$objective,aic=q.result$objective+sum(q.result$estimate!=0)*2,bic=q.result$objective+sum(q.result$estimate!=0)*log(length(y)),bij=result.global$bij,RI=result.global$eigenvector,R0=max(as.double(result.global$eivalues))))
}

func_boot_VZV<-function(Imp,boot){
  contact<-get(load(file=file.path(BM_contact_matrice_folder,paste0("contact_all_BM",".Rdata"))))
  contact_all<-contact[[boot]][,-1]
  time_use<-get(load(file=file.path(BM_TUD_matrice_folder,paste0("tij_general_BM",Imp,".Rdata"))))
  TUD<-time_use[[boot]]
  contact.result<-contact.fitter(a=a,y=y,rij1=contact_all, rij=TUD,muy=muy,N=N,D=D,Lmax=85,plots="TRUE",startpar=c(5e-2,0.84))
...
foreach(Imp=10:10,
        ...
        .errorhandling = 'remove')  %do% # transfer packages and run sequential  
  {
    foreach(boot = 1:1000,   # iteration index
      ...
      {
        result<-func_boot_VZV(Imp,boot)  
      }-> result
    filename = paste0("BM_combined_VZV_",Imp, ".txt")
