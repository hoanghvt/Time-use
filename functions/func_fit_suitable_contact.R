func_fit_suitable_contact<-function(cij,tij,initial_value){
  ...
  SEIR_model<-function(t,state,parms){
    ...
    with(as.list(c(state,parms)),{
      for(i in 1:5){
        dS[i]<- -beta*S[i]*sum(mij[i,]*(1-exp(-q2*tij[i,]/mij[i,]))*I/N)
        dE[i] <- beta*S[i]*sum(mij[i,]*(1-exp(-q2*tij[i,]/mij[i,]))*I/N) - (7)*E[i] ###latent period: 1 day
  ...
  model<- function(pars){
    out <- data.frame(ode(y = state, times = times, func = SEIR_model, parms = pars[1:2]))
    pop_matrix<-matrix(rep(N,52),ncol=5,byrow=T)
    ssr<-sum((pars[3]*out[,grepl("I",names(out))]/pop_matrix-obs_inc[,grepl("years old",names(obs_inc))]/pop_matrix)^2)
    return(ssr)
  }
  
  # initial_value<-  c(0.4,0.3,0.1)
  parms<-initial_value
  
  start<-Sys.time()
  Opt <- optim(parms,
               model,
               method = "L-BFGS-B",lower = c(0, 0,0),hessian = TRUE,
  ...
  names(results)<-c("beta","q2","scale","ls_value","R0")
  return(results)
}